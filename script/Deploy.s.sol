...
/**
 * @title DeployVestedBuy
 * @notice Deploys VestedBuy deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployVestedBuy is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(VestedBuyHook).creationCode;
        ...
        console2.log("VestedBuy ", predicted);
        ...
        vm.startBroadcast();
        VestedBuyHook hook = new VestedBuyHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("VestedBuy")),
